...
ТОЛЬКО загрузка данных, без расчёта корреляций.
"""
...
class HistoricalPricesFetcher:
    """Класс ТОЛЬКО для загрузки исторических данных"""
    ...
    def __init__(self, max_instruments: int = 50):
        ...
        print(f"🚀 Инициализация HistoricalPricesFetcher (макс. {max_instruments} инструментов)")
    ...
    def get_historical_data_sync(self, figi: str, days_back: int = 365) -> Optional[pd.DataFrame]:
        ...
        try:
            with Client(self.token) as client:
                ...
                if not data:
                    print(f"   ⚠️  Нет исторических данных для FIGI: {figi[:10]}...")
                    return None
    ...
    def fetch_all_historical_data(self, instruments_df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    def save_results(self, price_matrix: pd.DataFrame):
    ...
    def run(self):
        """Основной метод запуска - ТОЛЬКО загрузка данных"""
        print("=" * 70)
        print("📈 ЗАГРУЗКА ИСТОРИЧЕСКИХ ДАННЫХ (ШАГ 2)")
        print("ТОЛЬКО данные, без расчёта корреляций")
        print("=" * 70)
        ...
        files = self.save_results(price_matrix)
        
        # 6. Выводим итоговую статистику
        self.print_final_stats(price_matrix, files)
    
    def print_final_stats(self, price_matrix: pd.DataFrame, files: dict):
        """Выводит итоговую статистику"""
        print("\n" + "=" * 70)
        print("🎯 ИТОГОВАЯ СТАТИСТИКА ЗАГРУЗКИ ДАННЫХ")
        print("=" * 70)
        ...
        if self.metadata:
            ...
            print(f"   Макс дней: {metadata_df['data_points'].max()}")
            print(f"   Мин дней: {metadata_df['data_points'].min()}")
            print(f"   Среднее: {metadata_df['data_points'].mean():.0f}")
        ...
        print(f"\n🎯 Следующий шаг: python src/data_feed/calculate_correlations.py")
        print("=" * 70)
...
def main():
    ...
    try:
        ...
        fetcher = HistoricalPricesFetcher(max_instruments=30)  # Начнем с 30 инструментов
        fetcher.run()
